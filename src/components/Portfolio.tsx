          ...
          {categories.map((category) => (
            <button
              ...
              className={`px-6 py-3 rounded-full font-semibold transition-all duration-300 ${
                filter === category.key
                  ? 'bg-ider-yellow text-ider-dark neon-glow'
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {filteredProjects.map((project, index) => (
            ...
            >
              ...
              {/* Project Info */}
              <div className="p-6">
                <h3 className="text-xl font-bold mb-3 text-white group-hover:text-ider-yellow transition-colors duration-300">
                  {project.title}
                </h3>
                <p className="text-ider-grey mb-4 leading-relaxed">
                  {project.description}
                </p>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tech.map((tech, techIndex) => (
                    <span
                      key={techIndex}
                      className="px-3 py-1 bg-white/10 text-ider-yellow text-sm rounded-full"
                    >
                      {tech}
                    </span>
                  ))}
