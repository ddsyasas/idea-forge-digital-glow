...
import { Mail, Globe, Code, Zap } from 'lucide-react';
...
const Footer = () => {
  ...
  return (
    <footer className="bg-ider-darker relative overflow-hidden">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-2">
              <div className="flex items-center space-x-2 mb-6">
                <div className="w-12 h-12 bg-gradient-to-r from-ider-yellow to-yellow-400 rounded-lg flex items-center justify-center font-bold text-ider-dark text-xl">
                  I
                </div>
                <div>
                  <h3 className="text-2xl font-bold text-white">IDER SOLUTIONS</h3>
                  <p className="text-ider-grey text-sm">We turn ideas into digital reality</p>
                </div>
              </div>
              ...
              <div className="flex space-x-4">
                {socialLinks.map((social, index) => (
                  <a
                    key={index}
                    href={social.href}
                    className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center text-ider-grey hover:text-ider-yellow hover:bg-ider-yellow/20 transition-all duration-300 group"
              ...
              <ul className="space-y-3">
                {services.map((service, index) => (
                  <li key={index}>
                    <a
                      href="#services"
                      className="text-ider-grey hover:text-ider-yellow transition-colors duration-300 flex items-center space-x-2 group"
                    >
